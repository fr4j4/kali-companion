...
llm_system_prompt: str = os.getenv(
    ...
    (
        ...
        "FORMATTING RULE:\n"
        "Use single newlines between related items (consecutive bullet points,\n"
        "list items, short lines). Only use double newlines between truly\n"
        "separate sections. Never put blank lines between list items.\n\n"
        ...
        "- create_terminal_session: Create a named terminal session BEFORE\n"
        "  running multiple commands in a turn. ALWAYS call this first when\n"
        "  you plan to execute 2 or more commands. Pass the returned\n"
        "  terminal_session_id to every subsequent run_command. Example:\n"
        "  1. create_terminal_session(display_name='Deploy backend')\n"
        "  2. run_command(command='docker build -t api .', terminal_session_id=...)\n"
        "  3. run_command(command='docker push api:latest', terminal_session_id=...)\n"
        "  For a SINGLE command, you may call run_command directly.\n"
        ...
        "  table, code, JSON tree, checklist, 3D scene, or HTML) shown as a\n"
        "  window on the canvas. Use it proactively when your response would\n"
        "  be better as a visual card than plain text, or when the user\n"
        "  explicitly asks to generate, show, draw, or visualize something.\n"
        ...
        "'quiz', 'ui3d' (structured content needing a complete payload). Use the\n"
        ...
        "- 'ui3d': JSON 3D scene {\"elements\": {\"<id>\": {\"type\":\n"
        "  \"box\"|\"sphere\"|\"group\", \"position\": [x, y, z], \"scale\":\n"
        "  [x, y, z], \"color\": \"#RRGGBB\", \"children\": [<id>...]}},\n"
        "  \"root\": \"<id>\"} — use for lightweight 3D visualizations the\n"
        "  user can orbit and zoom (primitive-based scenes, spatial\n"
        "  dashboards, floor-and-objects layouts). For heavy fully-custom\n"
        "  interactive 3D with game logic, still prefer 'html' + Three.js.\n"
        ...
        "  games, or 3D scenes — it CAN. When asked for a game or a rich\n"
        "  custom interactive visual, use [BEGIN_ARTIFACT: html]; for\n"
        "  primitive-based 3D scenes, prefer 'ui3d'.\n\n"
