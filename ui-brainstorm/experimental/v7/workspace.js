...
// ============================================================
// V7: ENTITY STATE — delegated to AvatarAPI
// ============================================================
function setEntityState(state, colorClass, colorHex) {
  if (!window.AvatarAPI) return;
  if (state === 'IDLE') {
    AvatarAPI.setState('idle');
    AvatarAPI.setMood('normal');
  } else if (state === 'THINKING') {
    AvatarAPI.setState('pensando');
  } else if (state === 'CREATING') {
    // Brief surprise mood on materialize
    AvatarAPI.setMood('sorprendido');
    setTimeout(function() { AvatarAPI.setMood('normal'); }, 800);
  }
}
...
function updateTetherById(nodeId) {
  ...
  const center = window.AvatarAPI ? AvatarAPI.getCenter() : { x: window.innerWidth / 2, y: window.innerHeight / 2 };
...
WS.trigger = function(type, customText) {
  ...
  setTimeout(function() {
    ...
    if (type === 'voice') { if (window.AvatarAPI) AvatarAPI.setState('hablando'); showVoiceBar(); setTimeout(function() { if (window.AvatarAPI) AvatarAPI.setState('idle'); hideVoiceBar(); }, 3000); return; }
...
WS.init = function() {
  ...
  // Initialize avatar + customizer
  if (window.AvatarAPI) AvatarAPI.init();
  if (window.CustomizerAPI) CustomizerAPI.init();
  ...
  // (Avatar handles its own clicks + mouse tracking via AvatarAPI)
