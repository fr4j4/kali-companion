/* ============================================================
   SINGULARITY V6 — WIDGETS
   v5 base (17 widgets) + Kali Neural upgrades + 3 new
   ============================================================ */
...
WIDGETS.code = () => ({
  width: 380, height: 360, icon: '\u{1F4BB}', title: 'server.rs', resizable: true, minW: 300, minH: 200,
...
// ============================================================
// UPGRADED HERO (Kali Neural: HP/mana bars + skill panel)
// ============================================================
WIDGETS.hero = () => ({
  width: 340, height: null, icon: '\u{1F6E1}\uFE0F', title: 'ficha_pudge', resizable: true, minW: 280, minH: 380,
  ...
  init(root) {
    ...
    c.innerHTML = `
      <div class="bg-gradient-to-r from-red-950/70 to-black -mx-5 -mt-5 px-4 py-3 border-b border-white/10 flex items-center mb-4">
        <div class="w-10 h-10 rounded-lg bg-red-900/40 border border-red-500/60 flex items-center justify-center shrink-0 overflow-hidden relative">
          <span class="font-bold text-red-200 text-xs font-mono select-none">PDG</span>
        </div>
        <div class="ml-3">
          <h4 class="text-sm font-bold text-red-100 uppercase tracking-wider font-sans">Pudge</h4>
          <p class="text-[9px] font-mono text-red-400 uppercase tracking-widest">Melee \u2022 Durable \u2022 Disabler</p>
        </div>
      </div>
      <div class="flex justify-between items-center bg-black/40 p-2 rounded border border-white/5 text-center text-xs text-fg">
        <div class="w-1/3 border-r border-white/5"><span class="block text-red-500 font-bold text-sm">25 + 3.5</span><span class="text-[8px] text-muted uppercase font-mono">Fuerza \u2B50</span></div>
        <div class="w-1/3 border-r border-white/5"><span class="block text-green-500 font-bold text-sm">14 + 1.5</span><span class="text-[8px] text-muted uppercase font-mono">Agilidad</span></div>
        <div class="w-1/3"><span class="block text-blue-500 font-bold text-sm">16 + 1.5</span><span class="text-[8px] text-muted uppercase font-mono">Inteligencia</span></div>
      </div>
      <div class="space-y-1">
        <div class="flex justify-between text-[9px] font-mono text-muted"><span>PUNTOS DE VIDA</span><span class="text-green-400 font-bold">700 / 700 (+3.2)</span></div>
        <div class="hero-attr-bar"><div class="hero-attr-fill bg-gradient-to-r from-green-800 to-emerald-400" style="width:100%;"></div></div>
        <div class="flex justify-between text-[9px] font-mono text-muted mt-1"><span>MANA</span><span class="text-blue-400 font-bold">267 / 267 (+0.8)</span></div>
        <div class="hero-attr-bar"><div class="hero-attr-fill bg-gradient-to-r from-blue-800 to-cyan-500" style="width:100%;"></div></div>
      </div>
      <div class="space-y-2">
        <span class="text-[8px] font-mono text-muted uppercase tracking-wider block">Habilidades (Haz clic para ver detalles):</span>
        <div class="flex gap-2 justify-center pt-1 font-sans">
          <button class="hero-skill-btn w-9 h-9 rounded border border-red-500/40 bg-red-950/60 hover:bg-red-800/80 flex items-center justify-center text-sm transition active" data-skill="0" title="Meat Hook">\u{1FA9D}</button>
          <button class="hero-skill-btn w-9 h-9 rounded border border-green-500/40 bg-green-950/60 hover:bg-green-800/80 flex items-center justify-center text-sm transition" data-skill="1" title="Rot">\u2623\uFE0F</button>
          <button class="hero-skill-btn w-9 h-9 rounded border border-amber-500/40 bg-amber-950/60 hover:bg-amber-800/80 flex items-center justify-center text-sm transition" data-skill="2" title="Flesh Heap">\u{1F969}</button>
          <button class="hero-skill-btn w-9 h-9 rounded border border-red-500/40 bg-red-950/60 hover:bg-red-800/80 flex items-center justify-center text-sm transition" data-skill="3" title="Dismember">\u{1F52A}</button>
        </div>
        <div class="hero-skill-panel">
          <h5 class="aw-skill-title text-[10px] font-mono text-red-400 uppercase font-bold">\u{1FA9D} Meat Hook</h5>
          <p class="aw-skill-desc text-[9px] text-muted leading-relaxed mt-1">Lanza un gancho sangriento que arrastra a la primera unidad que golpea, infligiendo dano puro si es enemigo.</p>
        </div>
      </div>
      <div><div class="badge text-muted mb-1.5">sinergias</div><div class="flex gap-1.5 flex-wrap">
        <span class="text-[9px] px-2 py-0.5 rounded-full bg-red-500/15 text-red-300">Durable</span>
        <span class="text-[9px] px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300">Disabler</span>
        <span class="text-[9px] px-2 py-0.5 rounded-full bg-green-500/15 text-green-300">Regeneracion</span>
      </div></div>`;
    const skills = [
      { title: '\u{1FA9D} Meat Hook', desc: 'Lanza un gancho sangriento que arrastra a la primera unidad que golpea, infligiendo dano puro si es enemigo.', color: 'red' },
      { title: '\u2623\uFE0F Rot', desc: 'Emite una nube toxica que inflige dano continuo y ralentiza a los enemigos cercanos, danando tambien al propio Pudge.', color: 'green' },
      { title: '\u{1F969} Flesh Heap', desc: 'Concede resistencia al dano. Otorga fuerza adicional permanente cada vez que un heroe enemigo muere cerca.', color: 'amber' },
      { title: '\u{1F52A} Dismember', desc: 'CANALIZADA: Pudge devora a un enemigo, inmovilizandolo e infligiendo dano masivo basado en su propia fuerza.', color: 'red' }
    ];
    const titleEl = c.querySelector('.aw-skill-title');
    const descEl = c.querySelector('.aw-skill-desc');
    c.querySelectorAll('.hero-skill-btn').forEach(btn => {
      btn.onclick = () => {
        const idx = parseInt(btn.dataset.skill);
        const s = skills[idx];
        titleEl.textContent = s.title;
        titleEl.className = 'aw-skill-title text-[10px] font-mono text-' + s.color + '-400 uppercase font-bold';
        descEl.textContent = s.desc;
        c.querySelectorAll('.hero-skill-btn').forEach(b => { b.classList.remove('active'); b.classList.remove('border-red-400', 'bg-red-900/60', 'border-green-400', 'bg-green-900/60', 'border-amber-400', 'bg-amber-900/60'); });
        btn.classList.add('active');
        if (s.color === 'red') btn.classList.add('border-red-400', 'bg-red-900/60');
        else if (s.color === 'green') btn.classList.add('border-green-400', 'bg-green-900/60');
        else btn.classList.add('border-amber-400', 'bg-amber-900/60');
        if (window.beep) beep(600, 'sine', 0.08);
      };
    });
  }
});
...
WIDGETS.img = () => ({
  width: 260, height: null, icon: '\u{1F5BC}\uFE0F', title: 'concept_lyra.png', resizable: false,
...
WIDGETS.link = () => ({
  width: 300, height: null, icon: '\u{1F517}', title: 'enlace', resizable: false,
  body: '<a class="block p-4 hover:bg-white/[0.02] transition" href="https://crimsonvale.wiki/heroes/lyra" target="_blank" rel="noopener"><div class="flex items-start gap-3"><div class="w-11 h-11 rounded-xl bg-accent/15 border border-accent/20 flex items-center justify-center shrink-0"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-accent"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></div><div class="flex-1 min-w-0"><div class="text-sm font-semibold text-fg truncate mb-0.5">Wiki Oficial \u2014 Crimson Vale</div><div class="text-[11px] text-muted line-clamp-2 leading-relaxed mb-2">Documentacion completa de heroes, builds, lore y mecanicas de juego. Actualizado al parche 3.4.</div><div class="flex items-center gap-2"><div class="w-3 h-3 rounded-sm bg-accent/30"></div><span class="badge text-muted truncate">crimsonvale.wiki/heroes/lyra</span></div></div><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-faint shrink-0 mt-1"><path d="M7 7h10v10"/><path d="M7 17 17 7"/></svg></div></a>'
});

WIDGETS.markdown = () => ({
  width: 420, height: 500, icon: '\u{1F4DD}', title: 'guia_implementacion.md', resizable: true, minW: 320, minH: 280,
...
WIDGETS.mermaid = () => ({
  width: 400, height: 380, icon: '\u{1F500}', title: 'flow_auth.mmd', resizable: true, minW: 300, minH: 250,
...
// ============================================================
// UPGRADED MUSIC (Kali Neural: spinning vinyl + EQ bars)
// ============================================================
WIDGETS.music = () => ({
  width: 320, height: null, icon: '\u{1F3B5}', title: 'now_playing', resizable: false,
  body: '<div class="aw-music p-5 space-y-4"></div>',
  init(root) {
    const c = root.querySelector('.aw-music');
    c.innerHTML = `
      <div class="flex items-center gap-4">
        <div id="vinyl-disc" class="w-16 h-16 rounded-full border-4 border-black bg-neutral-900 relative flex items-center justify-center shadow-lg shrink-0 transition-transform duration-1000" style="background: repeating-radial-gradient(#151515 0, #151515 2px, #252525 3px, #252525 4px);">
          <div class="w-4 h-4 bg-accent rounded-full"></div>
          <div class="absolute w-1.5 h-1.5 bg-white rounded-full top-2"></div>
        </div>
        <div class="min-w-0 flex-1">
          <div class="text-sm font-semibold truncate">Neural Frequency Vol. II</div>
          <div class="text-[11px] text-muted truncate">Productor: Kali Core</div>
          <div class="badge text-faint mt-1">lossless \u2022 320kbps</div>
        </div>
      </div>
      <div>
        ...
        <div class="flex items-end justify-center gap-1 h-5 pt-1 aw-eq">
          <div class="eq-bar w-1 bg-accent rounded-t" id="eq-b1" style="height:4px;"></div>
          <div class="eq-bar w-1 bg-accent/80 rounded-t" id="eq-b2" style="height:12px;animation-delay:0.1s"></div>
          <div class="eq-bar w-1 bg-accent/60 rounded-t" id="eq-b3" style="height:6px;animation-delay:0.2s"></div>
          <div class="eq-bar w-1 bg-accent/80 rounded-t" id="eq-b4" style="height:14px;animation-delay:0.3s"></div>
          <div class="eq-bar w-1 bg-accent rounded-t" id="eq-b5" style="height:8px;animation-delay:0.15s"></div>
        </div>
      </div>
    ...
    const vinyl = root.querySelector('#vinyl-disc');
    ...
    if (playBtn) playBtn.onclick = () => {
      ...
      if (playing) {
        ...
        if (vinyl) vinyl.classList.add('spin-active');
        eqInterval = setInterval(() => { for(let i=1;i<=5;i++) { const el = document.getElementById('eq-b'+i); if (el) el.style.height = Math.floor(Math.random()*18)+3+'px'; } }, 100);
        timer = setInterval(() => {
        ...
        if (window.beep) beep(400, 'sine', 0.1);
      } else {
        playBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>';
        if (vinyl) vinyl.classList.remove('spin-active');
        if (eqInterval) clearInterval(eqInterval);
        for(let i=1;i<=5;i++) { const el = document.getElementById('eq-b'+i); if (el) el.style.height = '4px'; }
...
WIDGETS.longtext = () => ({
  width: 380, height: 420, icon: '\u{1F4C4}', title: 'transcript_session.txt', resizable: true, minW: 280, minH: 200,
...
WIDGETS.chart = () => ({
  width: 360, height: 340, icon: '\u{1F4CA}', title: 'latency_comparison', resizable: true, minW: 300, minH: 250,
...
WIDGETS.json = () => ({
  width: 360, height: 420, icon: '\u{1F5C2}\uFE0F', title: 'api_response.json', resizable: true, minW: 280, minH: 200,
...
WIDGETS.terminal = () => ({
  width: 380, height: 360, icon: '\u{1F5A5}\uFE0F', title: 'terminal \u2022 bash', resizable: true, minW: 280, minH: 200,
...
WIDGETS.video = () => ({
  width: 360, height: null, icon: '\u{1F3AC}', title: 'tutorial_websocket.mp4', resizable: false,
...
WIDGETS.diff = () => ({
  width: 380, height: 340, icon: '\u{1F527}', title: 'fix_null_ptr.patch', resizable: true, minW: 300, minH: 200,
...
WIDGETS.table = () => ({
  width: 380, height: 340, icon: '\u{1F4CB}', title: 'comparativa_modelos.csv', resizable: true, minW: 300, minH: 200,
...
WIDGETS.controls = () => ({
  width: 280, height: null, icon: '\u{1F39A}\uFE0F', title: 'config_modelo', resizable: false,
...
// ============================================================
// UPGRADED TEXT/PROSE (Kali Neural: font-size + bg-opacity sliders)
// ============================================================
WIDGETS.text = () => ({
  width: 380, height: null, icon: '\u270D\uFE0F', title: 'documento_conceptual', resizable: true, minW: 280, minH: 200,
  headerActions: copyBtn(() => {
    const el = document.querySelector('.aw.focused .prose-content');
    return el ? el.textContent : '';
  }, 'Copiar texto'),
  body: '<div class="aw-prose flex-1 overflow-hidden flex flex-col"></div>',
  init(root) {
    const c = root.querySelector('.aw-prose');
    const id = root.dataset.id;
    c.innerHTML = `
      <div class="px-4 py-2 bg-black/40 border-b border-white/5 flex gap-4 items-center text-[9px] font-mono text-muted uppercase">
        <div class="flex-1 flex items-center gap-2">
          <span>Fuente:</span>
          <input type="range" min="14" max="24" value="18" oninput="document.getElementById('${id}-text').style.fontSize=this.value+'px'" class="w-full">
        </div>
        <div class="flex-1 flex items-center gap-2">
          <span>Fondo:</span>
          <input type="range" min="40" max="95" value="70" oninput="document.getElementById('${id}').style.backgroundColor='rgba(12,12,18,'+(this.value/100)+')'" class="w-full">
        </div>
      </div>
      <div class="p-5 max-h-60 overflow-y-auto bg-black/10">
        <div id="${id}-text" class="text-sm font-sans leading-relaxed text-fg/90 prose-content">
          <span class="text-[10px] font-mono text-warn uppercase tracking-wider block mb-2">Analisis de Red Neural:</span>
          La idea central sobre <span class="text-warn font-medium">"arquitectura de baja latencia"</span> ha sido expandida con exito.
          Este modelo de interaccion descarta el scroll infinito repetitivo de los chats comunes de burbujas, forzando a que las respuestas sean objetos tridimensionales autonomos que se pueden distribuir a lo largo del lienzo.
        </div>
      </div>`;
  }
});
...
// ============================================================
// NEW: ITEM (Blink Dagger — Kali Neural)
// ============================================================
WIDGETS.item = () => ({
  width: 300, height: null, icon: '\u26A1', title: 'blink_dagger', resizable: false,
  body: '<div class="aw-item p-4 space-y-3"></div>',
  init(root) {
    const c = root.querySelector('.aw-item');
    c.innerHTML = `
      <div class="bg-gradient-to-r from-yellow-950/60 to-black -mx-4 -mt-4 px-4 py-3 border-b border-yellow-700/50 flex items-center justify-between mb-0">
        <div>
          <h4 class="text-xs font-bold text-yellow-500 uppercase tracking-wider font-sans">Blink Dagger</h4>
          <p class="text-[9px] font-mono text-yellow-700/80 uppercase tracking-widest font-semibold">Objeto de Movilidad</p>
        </div>
        <div class="flex items-center gap-1 bg-yellow-950/40 px-2.5 py-1 rounded text-yellow-400 text-[10px] font-mono border border-yellow-700/30">
          \u{1F4B0} 2250 Gold
        </div>
      </div>
      <p class="text-[11px] font-sans text-fg/90 leading-relaxed">
        <strong class="text-yellow-400">Activa: Blink.</strong> Teletransporta al portador de forma instantanea a una distancia maxima de 1200 unidades en la direccion seleccionada.
      </p>
      <p class="text-[10px] text-red-400/80 italic border-l-2 border-red-500/30 pl-2 leading-relaxed">
        El dano recibido por heroes enemigos o Roshan deshabilita el Blink durante 3.0 segundos.
      </p>
      <div class="flex justify-between items-center border-t border-white/5 pt-3 text-[10px] font-mono text-muted">
        <span class="flex items-center gap-1"><span class="text-yellow-500">CD:</span> 15.0s</span>
        <span class="flex items-center gap-1"><span class="text-blue-400">Mana:</span> 0</span>
      </div>
      <div class="pt-1">
        <button class="aw-blink-btn w-full py-1.5 bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 rounded text-[10px] font-mono uppercase tracking-wider transition font-sans">
          \u26A1 \u00A1Ejecutar Blink! (Salto en pantalla)
        </button>
      </div>`;
    const blinkBtn = c.querySelector('.aw-blink-btn');
    if (blinkBtn) blinkBtn.onclick = () => {
      const el = root;
      el.style.opacity = '0';
      el.style.transform = 'scale(0.8)';
      if (window.beep) beep(1000, 'sawtooth', 0.1);
      setTimeout(() => {
        const cx = window.innerWidth / 2, cy = window.innerHeight / 2;
        const angle = Math.random() * Math.PI * 2;
        const dist = 200 + Math.random() * 150;
        const w = el.offsetWidth, h = el.offsetHeight;
        el.style.left = (cx + Math.cos(angle) * dist - w/2) + 'px';
        el.style.top = (cy + Math.sin(angle) * dist - h/2) + 'px';
        el.style.opacity = '1';
        el.style.transform = 'scale(1)';
        if (window.updateTether) window.updateTether(root.dataset.id);
        if (window.triggerNetworkPulse) window.triggerNetworkPulse();
      }, 250);
    };
  }
});
...
// ============================================================
// NEW: LOCATION (Dire Jungle — Kali Neural)
// ============================================================
WIDGETS.location = () => ({
  width: 420, height: null, icon: '\u{1F5FA}\uFE0F', title: 'dire_jungle', resizable: false,
  body: '<div class="aw-location flex flex-col"></div>',
  init(root) {
    const c = root.querySelector('.aw-location');
    c.innerHTML = `
      <div class="h-32 w-full bg-cover bg-center relative select-none" style="background-image: url('data:image/svg+xml;utf8,<svg xmlns=\\'http://www.w3.org/2000/svg\\' width=\\'400\\' height=\\'200\\'><rect width=\\'400\\' height=\\'200\\' fill=\\'%23111827\\'/><path d=\\'M0 180 L120 70 L240 140 L360 50 L400 180 Z\\' fill=\\'%231f2937\\'/><path d=\\'M50 200 L180 110 L300 170 L400 100 L400 200 Z\\' fill=\\'%23111827\\' opacity=\\'0.5\\'/></svg>');">
        <div class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent"></div>
        <div class="absolute bottom-3 left-4">
          <h3 class="text-lg font-serif text-white tracking-wide">La Selva del Dire (Dire Jungle)</h3>
          <p class="text-[9px] text-accent font-mono">Frontera de Lava Roja</p>
        </div>
      </div>
      <div class="p-4 bg-black/80 space-y-3">
        <div class="flex justify-between gap-4 text-[10px] font-mono text-muted">
          <div class="flex flex-col"><span class="text-white font-medium">CLIMA</span>Neblina Acida</div>
          <div class="flex flex-col"><span class="text-white font-medium">AMENAZA</span>Critica (Extrema)</div>
          <div class="flex flex-col"><span class="text-white font-medium">FACCION</span>Radiant Control</div>
        </div>
        <div>
          <button class="aw-lock-btn w-full py-1.5 bg-accent/10 hover:bg-accent/20 text-accent text-[10px] font-mono uppercase tracking-wider rounded border border-accent/30 transition font-sans">
            \u{1F512} Fijar Coordenadas Neuroruta
          </button>
        </div>
      </div>`;
    const lockBtn = c.querySelector('.aw-lock-btn');
    if (lockBtn) lockBtn.onclick = function() {
      const orig = this.textContent;
      this.textContent = '\u{1F6F0}\uFE0F ESCANEANDO SATELITES...';
      this.classList.add('text-accent', 'border-accent');
      this.disabled = true;
      if (window.beep) beep(400, 'sine', 0.5);
      setTimeout(() => {
        this.textContent = '\u{1F512} RUTA FIJADA EN EL MAPA';
        this.classList.remove('text-accent');
        this.classList.add('text-ok', 'border-ok');
        if (window.beep) beep(800, 'sine', 0.1);
      }, 1500);
    };
  }
});
...
// ============================================================
// NEW: QR GENERATOR (Kali Neural)
// ============================================================
WIDGETS.qr = () => ({
  width: 260, height: null, icon: '\u{1F4F1}', title: 'qr_generator', resizable: false,
  body: '<div class="aw-qr p-4 flex flex-col items-center gap-4"></div>',
  init(root) {
    const c = root.querySelector('.aw-qr');
    c.innerHTML = `
      <div class="w-32 h-32 bg-white rounded-lg p-2.5 flex items-center justify-center shadow-lg transition-transform duration-300 hover:scale-105 select-none">
        <svg id="qr-matrix-svg" width="100%" height="100%" viewBox="0 0 100 100" fill="#000">
      ...
      <div class="w-full space-y-1.5">
        <span class="text-[8px] font-mono text-muted uppercase tracking-wider block">Codificar texto/URL:</span>
        <input type="text" value="https://dota2.com" class="aw-qr-input w-full bg-black/40 text-fg border border-white/10 rounded-lg py-1.5 px-2.5 text-xs outline-none focus:border-accent font-mono">
      </div>`;
    const input = c.querySelector('.aw-qr-input');
    const matrixGroup = c.querySelector('#qr-variable-dots');
    if (input && matrixGroup) input.oninput = () => {
      const textVal = input.value;
      const count = Math.min(25, 8 + textVal.length);
      let dots = '';
      for (let i = 0; i < count; i++) {
        const x = 35 + Math.floor(Math.random() * 55);
        const y = 35 + Math.floor(Math.random() * 55);
        dots += '<rect x="' + x + '" y="' + y + '" width="8" height="8"/>';
      }
      matrixGroup.innerHTML = dots;
    };
  }
});
